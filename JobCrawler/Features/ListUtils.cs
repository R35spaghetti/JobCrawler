...
public static class ListUtils
{
    ...
    public static string FilterJobAd(string jobAdInfo, List<string> keywords, string path,
        List<string> negativeKeywords, IWebDriver driver, string titleName, string headName, string subFolderName)
    {
        /*
        ...
        if (keywords.Count == 1 && negativeKeywords.Count == 1)
        {
            ...
            {
                FolderStructure.FolderStructureForAdsWithDates(jobAdInfo, path, driver, titleName,
                    headName, subFolderName);
                return jobAdInfo;
            }
        }
        else
    ...
    public static string FilterJobAd(string jobAdInfo, List<string> keywords, string path,
        List<string> negativeKeywords, IWebDriver driver, string titleName, string headName)
    {
        ...
        if (keywords.Count == 1 && negativeKeywords.Count == 1)
        {
            ...
            if (Regex.IsMatch(jobAdInfo.ToUpper(), $@"(?<=^|[\s\p{{P}}]){escapedStrNeg.ToUpper()}(?=[\s\p{{P}}]|$)"))
            {
                return string.Empty;
                         ...
                         $@"(?<=^|[\s\p{{P}}]){escapedStrPos.ToUpper()}(?=[\s\p{{P}}]|$)"))
            {
                FolderStructure.FolderStructureForAdsWithoutDates(jobAdInfo, path, driver, titleName, headName);
                return jobAdInfo;
            }
        }
        else
